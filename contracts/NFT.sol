...
function toString(uint value) internal pure returns (string memory) {
   ...
   function buildImage(uint _id, string[2] memory collateralNames, string[2] memory collateralAmounts, string memory lenderName, string memory lenderAmount, string memory status, string memory _type) pure public returns(string memory) {
    ...
    return Base64.encode(bytes(string(abi.encodePacked(
        ...
        string(abi.encodePacked(' <rect stroke="#2d2d2d" id="svg_10" height="75.07359" width="144.73118" y="157.83511" x="19.88808" stroke-width="3" fill="none"/> <text transform="matrix(0.653316 0 0 0.684165 17.3342 50.9147)" stroke="#2d2d2d" text-anchor="start"  font-size="30" id="svg_16" y="194.99114" x="19.38732" stroke-width="0" fill="#5A5A5A">Type</text> <text  text-anchor="start"  font-size="26" id="svg_17" y="220.04546" x="30.68199" stroke-width="0" stroke="#2d2d2d" fill="#848484">',_type,'</text> <rect stroke="#2d2d2d" id="svg_19" height="75.07359" width="144.73118" y="158.06586" x="175.39863" stroke-width="3" fill="none"/> <rect stroke="#2d2d2d" id="svg_20" height="75.07359" width="144.73118" y="158.56586" x="332.05246" stroke-width="3" fill="none"/>  <text style="cursor: move;"  text-anchor="start"  font-size="30" id="svg_23" y="219.04546" x="187.34866" stroke-width="0" stroke="#2d2d2d" fill="#DBB2E9" font-weight="900">',status,'</text> <text transform="matrix(0.653316 0 0 0.684165 17.3342 50.9147)" stroke="#2d2d2d"  text-anchor="start"  font-size="30" id="svg_24" y="195.08483" x="259.18957" stroke-width="0" fill="#5A5A5A">Status</text> <text style="cursor: move;" transform="matrix(0.653316 0 0 0.684165 17.3342 50.9147)" stroke="#2d2d2d"  text-anchor="start"  font-size="30" id="svg_25" y="196.30286" x="496.44074" stroke-width="0" fill="#5A5A5A">Loan ID</text>'
       )),

        string(abi.encodePacked('<text style="cursor: move;" stroke="#2d2d2d" text-anchor="start"  font-size="30" id="svg_27" y="219.04546" x="343.58673" stroke-width="0" fill="#848484">', toString(_id) ,'</text> <rect stroke="#2d2d2d" id="svg_28" height="88.15051" width="455.43879" y="247.83757" x="19.67837" stroke-width="3" fill="none"/> <text transform="matrix(0.653316 0 0 0.684165 17.3342 50.9147)" stroke="#2d2d2d" text-anchor="start"  font-size="30" id="svg_29" y="327.66268" x="21.74217" stroke-width="0" fill="#5A5A5A">Collateral</text> <text text-anchor="start"  font-size="30" id="svg_30" y="315.43008" x="31.45122" stroke-width="0" stroke="#2d2d2d" fill="#848484" >', collateralNames[0], collateralNames[1], '</text> <text text-anchor="start"  font-size="30" id="svg_31" y="316.19931" x="300.0666" stroke-width="0" stroke="#2d2d2d" fill="#AFAFAF" font-weight="900">', (collateralAmounts[0]), (collateralAmounts[1]),'</text>')),

        '<rect stroke="#2d2d2d" id="svg_32" height="88.15051" width="455.43879" y="347.83757" x="20.26316" stroke-width="3" fill="none"/> <text style="cursor: move;" transform="matrix(0.653316 0 0 0.684165 17.3342 50.9147)" stroke="#2d2d2d" text-anchor="start"  font-size="30" id="svg_33" y="475.53576" x="21.74217" stroke-width="0" fill="#5A5A5A">DEBT LEFT</text> <text style="cursor: move;" text-anchor="start"  font-size="30" id="svg_34" y="416.59967" x="32.03602" stroke-width="0" stroke="#2d2d2d" fill="#848484">',lenderName,'</text> <text text-anchor="start"  font-weight="900" font-size="30" id="svg_35" y="415.61451" x="300.42918" stroke-width="0" stroke="#2d2d2d" fill="#AFAFAF">',(lenderAmount),'</text>  </svg>'
    ))));
    }

    function getPrice(address _token, uint amount) internal view returns (string memory) {
     ...
     string memory amountToReturn;
        ...
        if(amount < (1 * 10 ** (units - 1))) {
            ...
            amountToReturn = string(abi.encodePacked(toString(wholeAmount), ".0", toString(fractionalAmount / 10 ** (units - 2))));
            }
        } else {
         amountToReturn = string(abi.encodePacked(toString(wholeAmount), ".", toString(fractionalAmount / 10 ** (units - 2))));
        }
    ...
    function getEncoded(uint tokenId) internal view returns(string memory) {
    ...
    string memory collateral_Name_1 = loan.collaterals[0] == address(0x0) ? "FTM" : collateral_1.symbol();
    string memory collateral_Name_2 = loan.collateralAmount.length > 1 ? loan.collaterals[1] == address(0x0) ? "FTM" : collateral_2.symbol() : "";  
    string memory lender_Name = loan.LenderToken == address(0x0) ? "FTM" : lender.symbol();
    
    string memory status = (loan.executed == true || loan.paymentsPaid == loan.paymentCount)? "Ended" : (loan.deadlineNext < block.timestamp ? "Default" : "Live");
    string memory _typeBorrow =  tokenId % 2 == 1 ? "Lender" : "Borrower";
    string[3] memory allAmounts = [getPrice(loan.collaterals[0], loan.collateralAmount[0]), loan.collateralAmount.length > 1  ? getPrice(loan.collaterals[1], loan.collateralAmount[1]) : "", getPrice(loan.LenderToken, loan.LenderAmount - loan.paymentsPaid * loan.paymentAmount)];
    return buildImage(_id, [collateral_Name_1, collateral_Name_2], [allAmounts[0], allAmounts[1]], lender_Name, allAmounts[2], status, _typeBorrow);
