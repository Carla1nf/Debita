...
function toString(uint value) private pure returns (string memory) {
   ...
   function buildImage(uint _id, string[2] memory collateralNames, string[2] memory collateralAmounts, string memory lenderName, string memory lenderAmount, string memory status, string memory _type) pure private returns(string memory) {
    ...
    function getPrice(address _token, uint amount) private view returns (string memory) {
    ...
    string[3] memory allAmounts = [getPrice(loan.collaterals[0], loan.collateralAmount[0]), loan.collateralAmount.length > 1  ? getPrice(loan.collaterals[1], loan.collateralAmount[1]) : "", getPrice(loan.LenderToken, loan.paymentAmount * (loan.paymentCount - loan.paymentsPaid))];
    return buildImage(_id, [collateral_Name_1, collateral_Name_2], (loan.executed ?  ["0", "/0"] : [allAmounts[0], allAmounts[1]] ), lender_Name, allAmounts[2], status, _typeBorrow);
